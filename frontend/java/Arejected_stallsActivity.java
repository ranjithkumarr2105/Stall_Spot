...
public class Arejected_stallsActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        swipeRefreshLayout.setOnRefreshListener(() -> fetchStalls("rejected"));
        fetchStalls("rejected");
    }
        ...
        ApiClient.getClient().create(ApiService.class).getStallsByStatus(status).enqueue(new Callback<List<AdminStall>>() {
            ...
            public void onResponse(@NonNull Call<List<AdminStall>> call, @NonNull Response<List<AdminStall>> response) {
                ...
                if (response.isSuccessful() && response.body() != null) {
                    ...
                    Toast.makeText(Arejected_stallsActivity.this, "Failed to load stalls.", Toast.LENGTH_SHORT).show();
                }
            }
            ...
            public void onFailure(@NonNull Call<List<AdminStall>> call, @NonNull Throwable t) {
                swipeRefreshLayout.setRefreshing(false);
                Toast.makeText(Arejected_stallsActivity.this, "Network Error: " + t.getMessage(), Toast.LENGTH_SHORT).show();
            }
        });
    }
}